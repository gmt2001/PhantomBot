$.on('command', function(event) {
	...
	if(command.equalsIgnoreCase("bid")) {
		...
		if(subCommand.equalsIgnoreCase("start")){ // !bid start command
			...
			$.say("/me Final notice! Current highest bidder is: " + $var.bid_highest_username + " with " + $var.bid_highest_amount + " " + $.pointname + " || DO I HAVE " + $var.highestincr + " ?")
            ...
            $.say("Bid is now ending, no more bid's will be added!");
            $.say("HIGHEST BIDDER IS: " + $var.bid_highest_username + " with " + $var.bid_highest_amount + " " + $.pointname + "!");
            $.say("/me CONGRATULATIONS");

            $var.bid_running = false;
            $.inidb.decr('points', $var.bid_highest_username.toLowerCase(), parseInt($var.bid_highest_amount));
            $.say($var.bid_highest_amount + " was withdrawn from " + $var.bid_highest_username + "'s account!");
            return;
		}else{
				...
				if($var.bid_running){
					...
					// IF this is not the first Bid!!!
						...
						if($var.user_bid < $var.highestincr){
							$.say(username + " Your bid should be " + $var.bid_increment + " bigger than heighest current bid!");
							return;
						}
						if($var.user_bid > $.inidb.get('points', sender)){
							$.say(username + " You don't have that much " + $.pointname + "!");
